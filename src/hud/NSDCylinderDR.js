...
export default class NSDCylinder extends BaseCylinder
{
    ...
    dibujarArcoInterior(giro)
    {
        ...
        this.ctx.lineWidth = this.r * 0.07;

        for (var i = 0; i < 4; i++) {
            this.ctx.beginPath();
            this.ctx.arc(0, 0, this.r * 1.5, giro + (Math.PI * i / 2), giro + (Math.PI * i / 2) + 1);
            this.ctx.stroke();
        }
    }
	...
	dibujarArcoExterior(giro)
    {
        ...
        this.ctx.lineWidth = this.r * 0.025;

        for (var i = 0; i < 2; i++) {
            this.ctx.beginPath();
            this.ctx.arc(0, 0, this.r * 1.0625, (Math.PI * i) - giro, (Math.PI * i) + 2.3 - giro);
            this.ctx.stroke();
        }
    }
